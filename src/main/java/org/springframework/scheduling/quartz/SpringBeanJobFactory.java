...
public class SpringBeanJobFactory extends AdaptableJobFactory implements ApplicationContextAware, SchedulerContextAware {
    ...
    protected Object createJobInstance(TriggerFiredBundle bundle) throws Exception {
        ...
        if (this.applicationContext == null) {
            ...
            // 由spring注入job对象，便于控制和对象重复利用
            job = this.applicationContext.getBean(bundle.getJobDetail().getJobClass());
            if (job == null) {
                this.applicationContext.getAutowireCapableBeanFactory().createBean(bundle.getJobDetail().getJobClass(), AutowireCapableBeanFactory.AUTOWIRE_CONSTRUCTOR, false);
            }
        }
